        ...
        for (tool in tools) {
            ...
            for ((runName, iterations) in runs) {
                for (iteration in iterations) {
                    ...
                    for (benchmarkName in benchmarks.sorted()) {
                        allJobs += async {
                            ...
                            allData += String.format(
                                ...
                                coverage.coverage.first().lines.covered.toInt(),
                                coverage.coverage.first().lines.total.toInt(),
                                coverage.coverage.first().lines.ratio * 100.0,
                                coverage.coverage.first().branches.covered.toInt(),
                                coverage.coverage.first().branches.total.toInt(),
                    ...
                    allJobs.awaitAll()
                    resultsDir.resolve("$tool-$runName-$iteration.csv").bufferedWriter().use {
